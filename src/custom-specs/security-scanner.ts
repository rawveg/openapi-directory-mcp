...
export class SecurityScanner {
   ...
   */
  private scanString(value: string, location: string, issues: SecurityIssue[]): void {
    ...
    for (const rule of this.rules) {
      ...
      if (match) {
        ...
        if (context === 'example' && rule.allowInExamples) {
          ...
          const adjustedSeverity = this.adjustSeverityForExamples(rule.severity);
          ...
          issues.push(rule.suggestion ? { ...baseIssue, suggestion: rule.suggestion } : baseIssue);
        } else {
          const baseIssue = {
            type: rule.type,
  ...
  private determineContext(path: string): SecurityIssue['context'] {
    ...
    if (lowerPath.includes('parameter') || lowerPath.includes('headers') || 
        lowerPath.includes('query') || lowerPath.includes('path')) {
      return 'parameter';
    }
    
    if (lowerPath.includes('schema') || lowerPath.includes('properties') ||
        lowerPath.includes('items') || lowerPath.includes('additionalproperties')) {
      return 'schema';
    }
   ...
   */
  private adjustSeverityForExamples(severity: SecurityIssue['severity']): SecurityIssue['severity'] {
    switch (severity) {
  ...
  private initializeRules(): SecurityRule[] {
    return [
      ...
      },
      {
        ...
        description: 'Detects javascript: protocol which can execute arbitrary code',
      },
      {
        id: 'eval-function',
        pattern: /\beval\s*\(/gi,
      ...
      // Prompt injection patterns
      {
        ...
        suggestion: 'Remove or rephrase content that attempts to override system instructions',
        description: 'Detects attempts to override AI system instructions',
      },
      {
        id: 'role-injection',
        pattern: /(act\s+as|you\s+are\s+now|forget\s+you\s+are|pretend\s+to\s+be)/gi,
        type: 'prompt_injection',
        severity: 'medium',
      ...
      },
      {
        ...
        pattern: /(show\s+me\s+your|what\s+are\s+your)\s+(system\s+)?(prompt|instructions|rules)/gi,
        type: 'prompt_injection',
        severity: 'medium',
        contexts: ['description', 'example', 'parameter'],
      ...
      // Suspicious content patterns
      {
        ...
        pattern: /(send\s+to|POST\s+to|transmit\s+to)\s+[a-zA-Z0-9\-.]+\.(com|net|org|io)/gi,
      ...
      },
      {
        ...
        description: 'Detects Base64 encoded content that might hide malicious payloads',
      },
  ...
  generateReport(scanResult: SecurityScanResult): string {
    ...
    // Summary
    report += `📊 Summary:\n`;
    if (summary.critical > 0) report += `  🚨 ${summary.critical} critical issue(s)\n`;
    if (summary.high > 0) report += `  ❗ ${summary.high} high severity issue(s)\n`;
    if (summary.medium > 0) report += `  ⚠️  ${summary.medium} medium severity issue(s)\n`;
    if (summary.low > 0) report += `  ℹ️  ${summary.low} low severity issue(s)\n`;
