...
exports.DualSourceApiClient = void 0;
const client_js_1 = require("./client.js");
const secondary_client_js_1 = require("./secondary-client.js");
const custom_spec_client_js_1 = require("../custom-specs/custom-spec-client.js");
 ...
 */
class DualSourceApiClient {
    constructor(primaryBaseURL, secondaryBaseURL, cacheManager) {
        this.primaryClient = new client_js_1.ApiClient(primaryBaseURL, cacheManager);
        this.secondaryClient = new secondary_client_js_1.SecondaryApiClient(secondaryBaseURL, cacheManager);
        this.customClient = new custom_spec_client_js_1.CustomSpecClient(cacheManager);
        this.cache = cacheManager;
    }
    async fetchWithCache(key, fetchFn, ttl) {
        const cached = this.cache.get(`triple:${key}`);
        if (cached) {
            return cached;
        }
    ...
    async getProviders() {
        return this.fetchWithCache(version_data_js_1.CACHE_KEYS.PROVIDERS, async () => {
            const [primaryProviders, secondaryProviders, customProviders] = await Promise.allSettled([
                this.primaryClient.getProviders(),
                this.secondaryClient.getProviders(),
                this.customClient.getProviders(),
            ]);
            const primary = primaryProviders.status === "fulfilled"
                ? primaryProviders.value
                : { data: [] };
            const secondary = secondaryProviders.status === "fulfilled"
                ? secondaryProviders.value
                : { data: [] };
            const custom = customProviders.status === "fulfilled"
                ? customProviders.value
                : { data: [] };
            // Merge all three sources
            const merged = merge_js_1.MergeUtilities.mergeProviders(primary, secondary);
            return merge_js_1.MergeUtilities.mergeProviders(merged, custom);
        });
    }
    /**
    ...
    async getProvider(provider) {
        return this.fetchWithCache(`${version_data_js_1.CACHE_KEYS.PROVIDER_PREFIX}${provider}`, async () => {
            ...
            const merged = merge_js_1.MergeUtilities.mergeAPILists(primary, secondary);
            return merge_js_1.MergeUtilities.mergeAPILists(merged, custom);
        });
    }
    /**
    ...
    async getServiceAPI(provider, service, api) {
        return this.fetchWithCache(`api:${provider}:${service}:${api}`, async () => {
            ...
            if (provider === "custom") {
                ...
                const hasCustomByName = await this.customClient.hasAPI(`custom:${service}:v1`);
                if (hasCustomByName) {
                    try {
            ...
            if (hasCustomAPI) {
                try {
                    return await this.customClient.getServiceAPI(provider, service, api);
                }
                catch (error) {
                    // Fallback to secondary/primary if custom fails
                }
    ...
    async getPaginatedAPIs(page = 1, limit = 50) {
        ...
        return this.fetchWithCache(cacheKey, async () => {
            ...
            const [primaryResults, secondaryResults, customResults] = await Promise.allSettled([
                pagination_js_1.PaginationHelper.smartFetch(async (page, limit) => {
                    const response = await this.primaryClient.getPaginatedAPIs(page, limit);
                    return {
                    ...
                    maxTotal: constants_js_1.PAGINATION.LARGE_FETCH_LIMIT,
                    chunkSize: constants_js_1.PAGINATION.CHUNKED_FETCH_SIZE,
                }).then((result) => ({
                    data: result.data,
            ...
            const primary = primaryResults.status === "fulfilled"
            ...
            const merged = merge_js_1.MergeUtilities.mergePaginatedAPIs(primary, secondary, page, limit);
            return merge_js_1.MergeUtilities.mergePaginatedAPIs(merged, custom, page, limit);
    ...
    async getMetrics() {
        return this.fetchWithCache("metrics", async () => {
            const [primaryMetrics, secondaryMetrics, customMetrics, primaryAPIs, secondaryAPIs, customAPIs,] = await Promise.allSettled([
            ...
            const primary = primaryMetrics.status === "fulfilled"
                ? primaryMetrics.value
                : { numSpecs: 0, numAPIs: 0, numEndpoints: 0 };
            const secondary = secondaryMetrics.status === "fulfilled"
                ? secondaryMetrics.value
                : { numSpecs: 0, numAPIs: 0, numEndpoints: 0 };
            const custom = customMetrics.status === "fulfilled"
                ? customMetrics.value
                : { numSpecs: 0, numAPIs: 0, numEndpoints: 0 };
            ...
            const customApiList = customAPIs.status === "fulfilled" ? customAPIs.value : {};
            // First merge primary and secondary
            const merged = await merge_js_1.MergeUtilities.aggregateMetrics(primary, secondary, primaryApiList, secondaryApiList);
            // Then merge custom on top (custom always wins)
            return await merge_js_1.MergeUtilities.aggregateMetrics(merged, custom, { ...primaryApiList, ...secondaryApiList }, customApiList);
        }, 300000); // Cache for 5 minutes to reflect custom spec changes
    }
    /**
    ...
    async searchAPIs(query, provider, page = 1, limit = 20) {
        ...
        return this.fetchWithCache(cacheKey, async () => {
            ...
            // Merge all three sources with custom taking highest precedence
            const merged = merge_js_1.MergeUtilities.mergeSearchResults(primary, secondary, query, page, limit);
            return merge_js_1.MergeUtilities.mergeSearchResults(merged, custom, query, page, limit);
        ...
        if (hasSecondary) {
            try {
                ...
                if (typeof this.secondaryClient.getAPIEndpoints === "function") {
                    return await this.secondaryClient.getAPIEndpoints(apiId, page, limit, tag);
                }
                else {
        ...
        if (url.startsWith("custom:")) {
            ...
            if (hasCustom) {
                try {
                    ...
                    if (!name || !version) {
                        throw new Error(`Invalid custom API ID format: ${url}. Expected format: custom:name:version`);
                    }
                    ...
                    throw new Error(`Failed to load custom API spec: ${error instanceof Error ? error.message : String(error)}`);
                }
            }
            else {
...
exports.DualSourceApiClient = DualSourceApiClient;
