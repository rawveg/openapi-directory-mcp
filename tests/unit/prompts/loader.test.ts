...
describe("PromptLoader", () => {
    ...
    it("should load prompts from all category folders", async () => {
      ...
      mockFs.statSync.mockImplementation(
        (dirPath) =>
          ({
            isDirectory: () =>
              !dirPath.toString().endsWith(".ts") &&
              !dirPath.toString().endsWith(".js"),
          }) as any,
      );
      ...
      const mockPrompts = {
        ...
        "authentication-guide": createMockPrompt(
          "authentication_guide",
          "core-discovery",
        ),
        "retrofit-api-client": createMockPrompt(
          "retrofit_api_client",
          "action-oriented",
        ),
        "api-auth-implementation": createMockPrompt(
  ...
  describe("getCategories", () => {
    it("should return prompts organized by category", async () => {
      ...
      expect(
        categories.find((c) => c.name === "category1")?.prompts,
      ).toHaveLength(1);
      expect(
        categories.find((c) => c.name === "category2")?.prompts,
      ).toHaveLength(2);
    });
  });
  ...
  describe("error handling", () => {
    ...
    it("should handle import errors gracefully", async () => {
      ...
      global.import = jest
        .fn()
        .mockRejectedValue(new Error("Import failed")) as any;
